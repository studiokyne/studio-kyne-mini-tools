...
**Baselines.** `phpcs.baseline.xml` gèle les constats PHPCS antérieurs à l'outillage : le code nouveau est tenu au standard, l'ancien ne bloque pas la CI. PHPStan tourne **sans baseline** depuis la PR #52, au niveau 6 depuis la PR #53 (types de tableaux dans les docblocs : `array<string, mixed>` pour les réglages et charges utiles, `string[]` pour les listes), **au niveau 8 depuis la PR #54** (unions `string|false` et nullables traités explicitement). **On s'arrête au niveau 8.** Mesure faite le 2026-09-19 : les niveaux 7 et 8 n'ajoutent qu'une trentaine de constats utiles (unions `string|false` passées à `esc_html()`, `foreach` sur `false`, appels sur nullable) ; les niveaux 9 et 10 doublent puis quadruplent le total en sanctionnant `mixed`, or WordPress renvoie `mixed` partout (`get_option()`, `get_post_meta()`, `$_POST`) — ce serait des centaines de casts défensifs sans gain. Suivi dans l'issue #50. Ne jamais régénérer une baseline pour faire passer un constat neuf — corriger, ou poser un `phpcs:ignore` / `@phpstan-ignore` motivé. La baseline PHPCS s'appaire sur le contenu des lignes et leur voisinage : toucher une ligne voisine d'un constat gelé le fait ressortir, et il faut alors le corriger ou l'annoter (jamais le regeler). Quand la baseline se vide, la supprimer. La famille Yoda a été soldée à la PR #55 (baseline 181 → 152), la famille nonce/assainissement des entrées à la PR #56 (152 → 84), les familles sortie non échappée, requêtes directes, `translators:` et `base64` à la PR suivante (84 → 55). Les messages d'exception de `FileManager` ne sont pas échappés à la source : ils partent en JSON et le toast les échappe à l'affichage. Deux angles morts de PHPCS sur cette famille : il ne voit pas un nonce vérifié dans une méthode d'aide (`guard()`, `check_nonce()`), d'où les `phpcs:ignore` / `phpcs:disable` motivés en tête des handlers AJAX ; et il ne reconnaît pas le transtypage `(int) ( $_POST['x'] ?? 0 )`, qu'il faut écrire `isset( $_POST['x'] ) ? (int) $_POST['x'] : 0`. Ne pas remplacer ce `(int)` par `absint()` : `absint(-5)` vaut 5, alors que le code traite les identifiants négatifs comme « aucun dossier ». Pour retirer les entrées devenues sans objet après une correction, générer une baseline neuve dans un fichier à part puis vérifier qu'elle ne contient aucun couple fichier/sniff absent de l'ancienne avant de la substituer. Le nom de classe du rapport contient des antislashs que le shell et Docker mangent (`Class file for report "DRCodeSnifferBaseline..." not found`) : le passer depuis un script entre apostrophes, `--report='\DR\CodeSnifferBaseline\Reports\Baseline' --report-file=nouvelle.xml --basepath=.`, en ayant écarté l'ancienne baseline le temps du passage.
