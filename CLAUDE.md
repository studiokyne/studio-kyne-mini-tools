...
Orchestrator (`Module.php`) over `ImageProcessor` (conversion/resize), `MediaLibrary` (media-list integration), `BulkProcessor` (batched WP-Cron optimization of the whole library), and `SvgHandler` (secure SVG upload support).
...
```
4. Add the class to `Activator::MODULE_CLASSES` (`includes/Core/Activator.php`) — the single list shared by activation defaults and `uninstall.php`
